...
- **Camera Capture** — Take photos directly from your browser on mobile devices; photos are downscaled in the browser to reduce upload time and storage; unsupported formats upload unchanged and the configured server size limit still applies
...
- Shooting never waits on the network: photos appear immediately, are saved on the device, and upload in the background through the resumable queue. Same-item photos keep their parent association across failures and reloads
...
| `MAX_IMAGE_BYTES` | `10485760` | Largest accepted upload, in bytes (direct and resumable), enforced by the server after browser resizing |
